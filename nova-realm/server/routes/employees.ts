...
export const getEmployees: RequestHandler = (_req, res) => {
  try {
    const employees = dbAll(
      `SELECT e.*, d.name as division, s.name as service, sec.name as section, eq.name as equipe
       FROM employees e
...
export const createEmployee: RequestHandler = (req, res) => {
  try {
    ...
    if (!matricule || !prenom || !nom || !division_id || !service_id || !section_id || !equipe_id) {
    ...
    const result = dbRun(
      `INSERT INTO employees (matricule, prenom, nom, division_id, service_id, section_id, equipe_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [matricule, prenom, nom, division_id, service_id, section_id, equipe_id]
    );
...
export const updateEmployee: RequestHandler = (req, res) => {
  try {
    ...
    dbRun(
      `UPDATE employees
       SET matricule = ?, prenom = ?, nom = ?, division_id = ?, service_id = ?, section_id = ?, equipe_id = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [matricule, prenom, nom, division_id, service_id, section_id, equipe_id, id]
    );
...
export const getSectionsByService: RequestHandler = (req, res) => {
  try {
    const { serviceId } = req.params;
    const sections = dbAll(
      `SELECT id, name FROM sections WHERE service_id = ? ORDER BY name`,
      [serviceId]
    );
    res.json(sections);
  } catch (err) {
...
export const getEquipesBySection: RequestHandler = (req, res) => {
  try {
    const { sectionId } = req.params;
    const equipes = dbAll(
      `SELECT id, name FROM equipes WHERE section_id = ? ORDER BY name`,
      [sectionId]
    );
    res.json(equipes);
  } catch (err) {
