...
// Map division and find correct IDs
function findOrganizationalIds(
  ...
  // Clean inputs
  const divClean = cleanText(division);
  const svcClean = cleanText(service);
  const secClean = cleanText(section);
  const eqClean = cleanText(equipe);
  ...
  if (!svc) {
    console.warn(`Service not found: ${service} in ${division}`);
    return null;
  }

  // Find section
  const sec = dbGet(
    `SELECT id FROM sections WHERE name = ? AND service_id = ?`,
    [secClean, svc.id]
  ) as any;
  if (!sec) {
    console.warn(`Section not found: ${section} in ${service}`);
    return null;
  }

  // Find equipe
  const eq = dbGet(
    `SELECT id FROM equipes WHERE name = ? AND section_id = ?`,
    [eqClean, sec.id]
  ) as any;
  if (!eq) {
    console.warn(`Equipe not found: ${equipe} in ${section}`);
    return null;
  }
...
export function parseEmployeesFromTSV(tsvData: string): RawEmployee[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const fonction = cleanText(row["Fonction"] || "");
...
export async function importEmployees(employees: RawEmployee[]) {
  ...
  for (const emp of employees) {
    try {
      ...
      if (!ids) {
        console.warn(`Could not find org structure for ${emp.matricule}`);
        skipped++;
        continue;
      }
