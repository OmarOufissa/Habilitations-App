...
export default function EditEmployee() {
  ...
  // Load sections when service changes
  useEffect(() => {
    if (formData.service_id) {
      loadSections(formData.service_id);
    }
  }, [formData.service_id]);

  // Load equipes when section changes
  useEffect(() => {
    if (formData.section_id) {
      loadEquipes(formData.section_id);
    }
  }, [formData.section_id]);
  ...
  const loadSections = async (serviceId: string) => {
    try {
      const response = await fetch(`/api/services/${serviceId}/sections`);
  ...
  const loadEquipes = async (sectionId: string) => {
    try {
      const response = await fetch(`/api/sections/${sectionId}/equipes`);
  ...
  const handleAddHabilitation = async () => {
    ...
    try {
      ...
      formData.append("type", newHabData.type);
      formData.append("codes", JSON.stringify(newHabData.codes));
      formData.append("numero", newHabData.numero);
      formData.append("date_validation", newHabData.dateValidation);
      if (newHabData.pdfFile) {
        formData.append("pdf", newHabData.pdfFile);
      }
      ...
      if (!response.ok) throw new Error("Erreur lors de l'ajout de l'habilitation");

      const newHab = await response.json();
      ...
      toast({
        ...
        description: "La nouvelle habilitation a été enregistrée",
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : "Erreur serveur";
      toast({
